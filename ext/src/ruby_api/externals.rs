...
impl Extern<'_> {
    ...
    fn inner_class(rb_self: Obj<Self>) -> Result<RClass, Error> {
        match rb_self.get() {
            Extern::Func(f) => Ok(f.class()),
            Extern::Global(g) => Ok(g.class()),
            Extern::Memory(m) => Ok(m.class()),
            Extern::Table(t) => Ok(t.class()),
        }
    }
}
